---
title: 第04讲：wait/notify/notifyAll 方法的使用注意事项
date: 2021-02-23 21:30:00
tags:
 - java
 - thread
categories:
 - concurrent
sidebar: true
pre: ./juc-03-thread-states
next: ./juc-05-product-consumer
---
...
* **然后**，此时生产者开始运行，执行了整个 give 方法，它往 buffer 中添加了数据，并执行了 notify 方法，但 notify 并没有任何效果，因为消费者线程的 wait 方法没来得及执行，所以没有线程在等待被唤醒。
* **最后**，刚才被调度器暂停的消费者线程回来继续执行 wait 方法并进入了等待。
