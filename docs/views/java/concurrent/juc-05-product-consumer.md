...
categories:
 - concurrent
sidebar: true
pre: ./juc-04-object-functions-analysis
next: ./juc-06-thread-safety-questions
---
...
![product-consumer.jpeg](./../../../images/product-consumer.jpeg)
...
```
如代码所示，最主要的部分仍是 take 与 put 方法，我们先来看 put 方法，put 方法被 synchronized 保护，while 检查队列是否为满，如果不满就往里放入数据并通过 notifyAll() 唤醒其他线程。
同样，take 方法也被 synchronized 修饰，while 检查队列是否为空，如果不为空就获取数据并唤醒其他线程。使用这个 MyBlockingQueue 实现的生产者消费者代码如下：
